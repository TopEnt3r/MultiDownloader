...
def title_search(query: str) -> int:
    ...
    # Collect data from soup - try new structure first, then old
    movie_divs = soup.find_all("div", class_="box")
    if not movie_divs:
        movie_divs = soup.find_all("div", class_="movie")
    
    for i, movie_div in enumerate(movie_divs):
        # Try new structure (titleFilm) first, then old (movie-title)
        title_tag = movie_div.find("h2", class_="titleFilm")
        if not title_tag:
        ...
        # Try to get image
        img_tag = movie_div.find('img', class_='layer-image')
        if not img_tag:
