...
class MPD_Segments:
    ...
    async def _concatenate_segments_in_order(self, temp_dir, concat_path, total_segments):
        ...
        # Build list of existing segment files in order
        segment_files = []
        ...
        if not segment_files:
            console.print(f"[yellow]No segment files found in {temp_dir}[/yellow]")
            return
        
        console.print(f"[cyan]Concatenating {len(segment_files)} segments in {temp_dir} -> {concat_path}[/cyan]")
        
        # Use cat with shell on Unix for speed
        if platform.system() != 'Windows':
            try:
                ...
                # Use shell to concatenate all files
                ...
                console.print(f"[green]Concatenation complete: {os.path.getsize(concat_path)} bytes[/green]")
                console.print(f"[green]Encrypted file ready at {concat_path}[/green]")
                return
                
            except Exception as e:
                console.print(f"[yellow]Fast concat failed: {e}, using fallback[/yellow]")
        
        # Fallback: Python method with large buffer
        with open(concat_path, 'ab', buffering=4*1024*1024) as outfile:
            for seg_file in segment_files:
                with open(seg_file, 'rb') as infile:
                    shutil.copyfileobj(infile, outfile, length=4*1024*1024)
