...
class MPDParser:
    ...
    def select_video(self, force_resolution="Best"):
        ...
        force_resolution_l = (force_resolution or "Best").lower().replace("p", "").replace("px", "")
        ...
        elif force_resolution_l.isdigit():
            # Find video with matching or closest height.
            # Prefer representations <= target, otherwise pick the smallest above target.
            target_height = int(force_resolution_l)

            def height_val(rep):
                return rep.get('height', 0) or 0

            lower_or_equal = [r for r in video_reps if height_val(r) and height_val(r) <= target_height]
            higher = [r for r in video_reps if height_val(r) and height_val(r) > target_height]

            if lower_or_equal:
                selected_video = max(lower_or_equal, key=lambda r: (height_val(r), r.get('bandwidth', 0)))
                filter_custom_resolution = f"{height_val(selected_video)}p"
            elif higher:
                selected_video = min(higher, key=lambda r: (height_val(r), r.get('bandwidth', 0)))
                filter_custom_resolution = f"{height_val(selected_video)}p*"
            else:
                selected_video = MPDParser.get_worst(video_reps)
                filter_custom_resolution = "Worst"
