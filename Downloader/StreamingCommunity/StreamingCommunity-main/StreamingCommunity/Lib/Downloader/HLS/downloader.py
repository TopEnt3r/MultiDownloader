...
def get_force_resolution():
    """Get force_resolution dynamically from config file"""
    config_manager.reload()  # Reload config to get latest value
    return str(config_manager.get('M3U8_CONVERSION', 'force_resolution')).strip().lower()
...
class M3U8Manager:
    ...
    def select_streams(self):
        ...
        if not self.is_master:
            ...
            # Video selection logic - get resolution dynamically
            current_resolution = get_force_resolution()
            if str(current_resolution) == "best":
                self.video_url, self.video_res = self.parser._video.get_best_uri()
            elif str(current_resolution) == "worst":
                self.video_url, self.video_res = self.parser._video.get_worst_uri()
            elif str(current_resolution).replace("p", "").replace("px", "").isdigit():
                resolution_value = int(str(current_resolution).replace("p", "").replace("px", ""))
    ...
    def log_selection(self):
        ...
        def calculate_column_widths():
            ...
            data_rows.append(["Video", available_video, str(get_force_resolution()), downloadable_video])
